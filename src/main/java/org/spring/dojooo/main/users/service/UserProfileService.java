...
public class UserProfileService {
    ...
    // 프로필 수정
    @Transactional
    public void editProfile(Long userId, ProfileUpdateRequest profileUpdateRequest, Authentication authentication) {
        User user = userEqualsCurrentUser(userId, authentication); // 현재 로그인한 사용자 검증
        ...
        Set<String> existingTagNames = findAllTags.stream()
                .map(ProfileTag::getTagName)
                .collect(Collectors.toSet());

        if (selectedTags != null) {
            validateSelectedTags(selectedTags, existingTagNames); // 중복 및 존재 여부 검증
        }

        for (ProfileTag profileTag : findAllTags) {
            String tagName = profileTag.getTagName();
    ...
    public void resetProfile(Long userId, Authentication authentication) {
        ...
        if (currentProfile != null) {
            String currentImageUrl = currentProfile.getProfileImage();
            // 기존 이미지가 Default 이미지(기본 이미지)가 아닌 등록한 이미지인 경우 S3에 등록 이미지 제거
        ...
        List<ProfileTag> usersTags = profileTagRepository.findAllByUser(user);
        for (ProfileTag profileTag : usersTags) {
            profileTag.settingShowOnProfile(false);
        }
    ...
    // 태그 중복 및 존재 여부 검증
    private void validateSelectedTags(List<String> selectedTags, Set<String> existingTagNames) {
        // 중복 검사 - HashSet (중복을 허용하지 않음)
